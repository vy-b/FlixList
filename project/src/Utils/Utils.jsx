...
function getUser(username, password) {
    return new Promise((resolve, reject) => {
        axios.get('http://localhost:3001/getUser', { params: { username: username, password: password } }).then((response) => {
            if (response.status === 200) {
                resolve(response);
            } else {
                reject(response);
...
export { addFriend, getFriends, signup, login, getUser, addRating, getRatings, getSearchResults, getMovieDetails }